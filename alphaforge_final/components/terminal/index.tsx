...
const TABS = ["Home", "Strategy", "Events", "Portfolio", "Trades", "Library"] as const
...
export function Terminal() {
  ...
  // Build the pendingResult for the Library save button
    ...
    <div className="flex h-screen flex-col overflow-hidden" style={{ background: C.bg }}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="border px-2 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em]"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="border px-2 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em]"
                style={{ color: C.text, borderColor: C.dim }}>
          ...
          {user && (
            <button
              ...
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = C.body }}
            >
              Log out
            </button>
          )}
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar
          ...
          onRun={handleRun}
          loading={loading}
          onTickerSelect={ticker => handleChange({ ticker })}
        />
        ...
        <main className="flex flex-1 flex-col overflow-hidden" style={{ background: C.bg }}>
          {params.mode === "live" ? (
            ...
            <TabHome data={data} loading={loading} error={error}
                     ticker={params.ticker} strategy={params.strategy} />
          ) : activeTab === "Strategy" ? (
            ...
            <TabLibrary pendingResult={pendingResult} />
          )}
        </main>
      </div>
    </div>
