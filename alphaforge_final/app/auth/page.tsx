  ...
  return (
    <>
      {corners.map((c, i) => (
        <span
          key={i}
          className={`absolute w-4 h-4 opacity-30 pointer-events-none ${c}`}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center w-full px-6">
        <Reveal delay={0.1}>
          <p
            className="font-mono text-[9px] tracking-[0.35em] uppercase mb-8 text-center"
            style={{ color: C.muted }}
          >
        ...
        <Reveal delay={0.2}>
          <h1
            className="font-serif text-5xl font-extrabold tracking-tight mb-1 text-center"
          ...
          >
            {confirmSent ? (
              <div className="text-center">
                ...
                <button
                  onClick={() => { setMode("signin"); setConfirmSent(false) }}
              ...
              <>
                <div className="flex mb-8 border" style={{ borderColor: C.dim }}>
                  {(["signin", "signup"] as const).map((m) => (
                    <button
                      ...
                      className="flex-1 py-2 font-mono text-[9px] tracking-[0.25em] uppercase transition-colors duration-150"
                      style={{
                        background: mode === m ? C.text : "transparent",
                        color: mode === m ? C.bg : C.mid,
                        borderRight: m === "signin" ? `1px solid ${C.dim}` : undefined,
                      }}
                    >
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  {error && (
                    <div
                      className="font-mono text-[10px] border px-3 py-2"
                      style={{ color: C.neg, borderColor: C.neg, background: C.negDim }}
                    >
                      {error}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!loading) {
                        e.currentTarget.style.background = C.bright
        ...
        <Reveal delay={0.6}>
          <p
            className="font-mono text-[8px] tracking-[0.2em] uppercase mt-8"
            style={{ color: C.muted }}
          >
